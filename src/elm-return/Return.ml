...
(*| *)
type ('msg,'model) return =
    ( 'model * 'msg list )


(*| *)
type ('msg,'model) returnF =
    ('msg,'model) return -> ('msg,'model) return


(*| *)
(* piper : List (ReturnF msg model) -> ReturnF msg model *)
let piper l =
    List.fold_left (<<) zero l
...
let sequence l =
    let
        f ( model, cmd ) ( models, cmds ) =
            ((model :: models) , List.concat [ cmd ; cmds ] )
    in
        List.fold_right f l ( [], [] )
...
let chain u l r =
    List.fold_left (fun r e -> r |> andThen (u e)) r l
